...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'setup-verify') {
    ...
    await interaction.editReply('✅ Verification system set up! Channel locked, role created, and verify message posted.');
  }
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (!verifiedRole) {
    return interaction.reply({
      content: '⚠️ Verified role not found. Please ask an admin to run `/setup-verify`.',
      ephemeral: true,
    });
  }
  ...
  try {
    ...
    await interaction.reply({
      content: '❌ Something went wrong. Make sure the bot role is above the Verified role in Server Settings.',
      ephemeral: true,
    });
  }
});
